...
void fight(Entity &aE1, Entity &aE2) {
    ...
    for(int i=0; i < 12 && !aE1.dead() && !aE2.dead(); ++i) {
        ...
        if(p1 > p2) {
            const int hit = p1 - p2;
            std::cout << aE1.name << " hit for " << hit << " hp!" << std::endl;
            aE2.hp -= hit;
        }
        else if(p2 > p1) {
            const int hit = p2 - p1;
            std::cout << aE2.name << " hit for " << hit << " hp!" << std::endl;
            aE1.hp -= hit;
        }
        else {
            std::cout << aE1.name << " and " << aE2.name << " were an even match!" << std::endl;
        }
    }
    if(aE1.dead())
        ...
        std::cout << aE1.name << " and " << aE2.name << " decide to have a beer instead!" << std::endl;
}
